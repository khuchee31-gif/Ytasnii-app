...
void main() {
  testWidgets('Код нь «412-995» хэлбэрээр, тараахаас ӨМНӨ гарна', (
    ...
    await pumpScreen(tester, FairnessScreen(controller: c, onSealed: () {}));
  ...
  testWidgets('Уншигч суудал нь утас барьсан хүн БИШ, hash-аас гарна', (
    ...
    expect(
      find.text('Утас барихгүй хүн уншиж, дэвтрийнхээ буланд бич.'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('Сэгсрэлт 2.0 секунд хуримтлагдаж, дараа нь л тараана', (
    ...
    await pumpScreen(
      tester,
      FairnessScreen(controller: c, onSealed: () => sealed = true),
    );

    await tester.tap(find.text('Бичиж авлаа'));
    await tester.pump();
    expect(
      find.text('№${c.previewReaderSeat} — утсаа хоёр секунд сэгсэр.'),
      findsOneWidget,
    );

    final TestGesture g = await tester.startGesture(
      tester.getCenter(find.byKey(const ValueKey<String>('shakePad'))),
    );
