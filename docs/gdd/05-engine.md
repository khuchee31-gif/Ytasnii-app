...
|---|---|
| `YOU_HEALED_SOMEONE` | v1-д мафи шөнө бүр **заавал** нэгийг оносон байдаг (алгасалт байхгүй, таймер байхгүй, мафи амьд байхгүй бол тоглоом аль хэдийн дууссан). Тиймээс **`NIGHT_NO_KILL` ⇔ Эмчийн бай довтлогдсон.** Эмч нийтийн мөрнөөс өөрөө мэднэ. |
| `ATTACKED_BUT_SURVIVED` | Дээрхтэй ижил логик. Хохирогч нь өөрөө Эмчийн байг таана — гэхдээ бүх ширээ ч тааж байна, тиймээс энэ нь **мэдээлэл биш, маргаан** болж хувирна. |
...
Үр дүн: **`NightReport.privateMsgs` нь v1-д хоосон, эсвэл Мөрдөгчийн нэг бичлэгтэй.** Инвариант I12 үүнийг хатуу шалгана — хэрэв өөр суудал мессеж авбал хэн нэг нь v2-ын дүрийг v1 руу чимээгүйхэн оруулсан гэсэн үг.
...
List<Cue> buildCues(_Work w) {
  final c = <Cue>[];
  if (w.deaths.isEmpty) {
    c.add(Cue.line('NIGHT_NO_KILL'));                 // «Өнөө шөнө хохирогч гарсангүй.»
  } else {
    c.add(Cue.line('NIGHT_KILL_A'));                  // «Өнөө шөнө…»
    c.add(Cue.silence(2500, duckAmbienceDb: -40));     // ХАТУУ. I11 шалгана.
    c.add(Cue.line('NIGHT_KILL_B', seat: w.deaths.first.victim));
  }
  if (w.whisper.length == 2) c.add(Cue.line('WHISPER_TWO', seats: w.whisper));
  else if (w.whisper.length == 1) c.add(Cue.line('WHISPER_ONE', seats: w.whisper));
  // 0 бол ямар ч мөр байхгүй — «Хот чимээгүй байна» гэсэн мөр ЗОРИУД байхгүй,
  // тэр нь шивнээ ажиллаагүйг зарлах болно.
  return c;
}
```
...
| ID | Инвариант |
|---|---|
| I1 | Лацдах мөчид үхсэн байсан суудлаас үйлдэл хэрэгжихгүй |
| I2 | `(actor, ability)` хос тутамд хамгийн ихдээ нэг үйлдэл |
| I3 | `deaths ⊆ aliveAtSeal`, `aliveAfter.length == aliveAtSeal.length - deaths.length` |
| I4 | **`deaths.length ≤ 1`** — v1-д довтолгоо нэг л байна |
| I5 | Үхэл бүрд `lethal(attack, defSnapshot[victim])` үнэн; эдгээлт нь `basic` довтолгоог **чанд** цуцална |
| I6 | Ижил оролтоор хоёр удаа дуудвал `resultHash` **ба** canonical байт ижил |
| I7 | Шийдвэрлэх үед `DateTime`/`Random` уншигдахгүй (тестэд алдаа шиддэг stub-аар сольж албадна) |
| I8 | `infoAnswer` товших мөчид өгсөн хариу = тайлан дахь хариу |
| I9 | 20–80, 110, 140–150 хувин **хоосон**; `AttackLevel.powerful` хэзээ ч гарахгүй |
| I10 | `whisper.length ≤ 2`, өсөх дараалал, бүх гишүүн `aliveAfter` дотор, тоолол ≥ `whisperMinAgree` |
| I11 | `deaths.length == 1` бол `cues` дотор `[killA, silence(2500), killB]` **зэрэгцээ, тэр дарааллаар** |
| I12 | `privateMsgs.keys ⊆ {амьд Мөрдөгчийн суудал}` |
| I13 | `win == mafi` ба `win == hotynhon` хэзээ ч зэрэг биш; `alive` хоосон биш |
| I14 | `pipsForDay`-ийн гарын үсэг нь `roleBySeat`-ыг **авдаггүй** (компайлын үеийн шалгалт) |
| I15 | Суудал бүр шөнө бүр яг нэг `Intent` илгээсэн (амьд суудлын тоо = лацдсан санааны тоо) |
...
**Бодит хуваарь, оюутны чадвараар:** `flutter test`-ийн дотор **2,000 seed** (< 10 секунд, PR бүрд). `dart tool/fuzz.dart` нь **200,000 seed** (~3 минут зөөврийн компьютер дээр), **хаалга бүрийн өмнө нэг удаа** ажиллана. `docs/03`-ын «шөнө бүр 1 сая seed» гэсэн зөвлөмж нь CI дэд бүтэц шаардана — энэ төсөлд түүнийг **хасаж байна**, орондоо хоёр бодит тоог барьж байна.

Нэмэлт хоёр зүйл, хямд бөгөөд их үнэ цэнтэй:

- **Golden snapshot.** 12 тогтоосон шөнө (§11-ийн жишээ ба 11 захын тохиолдол) → хүлээгдэж буй `NightReport`-ын canonical JSON нь repo-д commit хийгдэнэ. Шийдвэр чимээгүйхэн өөрчлөгдвөл diff гарна. **~0.5 өдөр, бүх багцын хамгийн өндөр өгөөжтэй тест.**
...
5. **130 `info`.** P9 нь P11-ийг шалгана. P11 = Алуурчин → `Faction.mafi` → **«Мөр олдлоо.»** Энэ мөр нь P9-ийн гарт, эргэлтийн дундуур **аль хэдийн харагдсан**; хувин нь зөвхөн тайланд бичээд `assert` хийнэ (I8).
6. **135 `whisper`.** Бүх товшилтын сан: **5 → 3** (P1, P6, P10) · **9 → 3** (P2-ийн алалт, P4-ийн эдгээлт, P11-ийн хаягдсан алалт) · **2 → 2** (P8, P12) · **4 → 1** (P5) · **11 → 1** (P9). Өнөө шөнө хэн ч үхээгүй тул хасалт байхгүй. Босго = 3 → `{5: 3, 9: 3}`. Тэнцэл: `rank(P9)=0 < rank(P5)=8`, тиймээс хоёулаа сонгогдоно. Хэвлэх нь **суудлын дугаараар**: **`whisper = [5, 9]`**.
7. **160 `cues`.** `[line NIGHT_NO_KILL, line WHISPER_TWO(5, 9)]`. Үхэл байхгүй тул 2.5 секундын блок байхгүй.
...
| Шударга байдлын код (гарал + харуулах + сэгсрэлт) | **v1** | **1.0** |
| `resolveNight` — 8 хувин, `pickVictim`, `topWhisper`, `buildCues` | **v1** | **3.0** |
| `b₀`, `pipsForDay`, `evaluateWin` + `b₀ < 0` татгалзал | **v1** | **0.5** |
| Үйл явдлын бүртгэл (SQLite, синхрон бичилт, дахин тоглуулалт) | **v1** | **2.0** |
| `resolveIdempotent` + `EngineStateCorrupt` → бүтэн дахин тоглуулалт | **v1** | **1.0** |
| Инвариант I1–I15 + шинжийн тест + 2,000 seed-ийн fuzz | **v1** | **2.5** |
| Golden snapshot (12 шөнө) | **v1** | **0.5** |
| Тоглолтын дараах «Шударга байдлын шалгалт» дэлгэц | **v1** | **1.0** |
...
**v1-ийн нийт: 14.75 хөгжүүлэлтийн өдөр ≈ 118 цаг.** Энэ нь M1 (Хөдөлгүүр CLI дээр, W03–W06) ба M2-ын хөдөлгүүрийн хэсэгт хуваагдана. Хэрэв хугацаа шахвал **хамгийн түрүүнд хасах зүйл нь дифференциал тест биш, «Шударга байдлын шалгалт» дэлгэцийн гоё хэлбэр** (текстийн жагсаалт болгож хямдруулна, 1.0 → 0.25).
...
4. **`EngineStateCorrupt` бодитоор хэд гарах вэ?** Зээлсэн Redmi ба Huawei дээр шөнийн эргэлтийн дундуур апп үхэх нь **үндсэн тохиолдол**, захын тохиолдол биш. **Хэмжих нь:** M2.5-д эргэлтийн дундуур утсыг 10 удаа хүчээр хаа. Хэрэв бүтэн дахин тоглуулалт 1 секундээс их болвол `night_result` кэшийг заавал ажиллуул; хэрэв `inputHash` зөрөх нь илэрвэл `IntentSubmitted`-ын бичилт синхрон биш байна гэсэн үг.
5. **`AttackLevel.powerful`-ыг enum дотор одоо байлгах нь зөв үү?** I9 нь v1-д хэрэглэгдэхгүйг шалгана, тиймээс зардал нь тэг. Гэвч хэрэв v2 хэзээ ч ирэхгүй бол энэ нь уншигчийг төөрөгдүүлэх хоосон код. **Шийдвэр:** байлга. Хасах нь 5 минут, дараа нь нэмэх нь `lethal`-ын бүх тестийг дахин бичих.
